...
# 6. Verify Azure Monitor OpenTelemetry is working
kubectl logs -l app=ml-api | grep "Azure Monitor OpenTelemetry configured successfully"

# 7. Test and generate telemetry
for i in {1..20}; do
...
```python
# Add to existing imports at the top of the file (after the existing imports)
from azure.storage.blob import BlobServiceClient
import io

# Add storage client configuration after the TELEMETRY_ENABLED setup section
# (after the "tracer = None" fallback lines)
...
# Replace the existing "model = joblib.load('model.pkl')" line with:
model = load_latest_model()

# Add these new endpoints to the existing API
# (add them after the existing /metrics endpoint, before the @app.on_event("shutdown") handler)
